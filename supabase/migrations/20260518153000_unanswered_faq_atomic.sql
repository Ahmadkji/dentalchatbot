-- Atomic unresolved→FAQ flow: update unanswered row + optionally insert FAQ in one transaction.
-- Prevents partial state if FAQ insert fails after the unanswered row is already updated.
...
begin
  ...
  set status = coalesce(p_status, status),
      answer = case when p_answer is null then answer else p_answer end,
      updated_at = now()
  where id = p_question_id
    and clinic_id = p_clinic_id
  ...
  if p_add_to_faq then
    -- avoid duplicate FAQ insert on retries
    select fe.id
    into v_faq_id
    ...
    if v_faq_id is null then
      -- serialize sort_order generation per clinic for this tx
